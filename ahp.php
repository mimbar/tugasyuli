...
function ahp_get_matrik_kk($id_kriteria, $id_kk){
	global $con;
	for($i=0;$i<count($id_kk);$i++){
		for($ii=0;$ii<count($id_kk);$ii++){
			if($i==$ii){
				$matrik[$i][$ii]=1;
			}else{
				if($i < $ii){
					$q=mysql_query("select nilai from nilai_pemohon where id_kriteria='".$id_kriteria."' and id_pemohon_1='".$id_kk[$i]."' and id_pemohon_2='".$id_kk[$ii]."'");
					if(mysql_num_rows($q)>0){
						$h=mysql_fetch_array($q);
						$nilai=$h['nilai'];
						$matrik[$i][$ii]=$nilai;
